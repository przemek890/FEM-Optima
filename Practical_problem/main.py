...
if __name__ == "__main__":
    ...
    start_index = next(i for i, line in enumerate(lines) if '*Node' in line)
    end_index = next(i for i, line in enumerate(lines) if '*Element, type=DC2D4' in line)

    new_lines = lines[:start_index + 1] + [process_section(lines[start_index + 1:end_index])] + lines[end_index:]
